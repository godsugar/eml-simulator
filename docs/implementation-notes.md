...
- サンプル点: 実軸上の 10 点 `[0.2, 0.4, 0.6, 0.8, 0.9, 0.3, 0.7, 1.0, 1.5, 2.0]`
  - 0 と負の値を避けている（ln / sqrt のドメイン制限を回避）
  - arcsin / arccos のドメイン（−1〜1）をカバーするため 1 以下の値を多めにサンプリング
- 候補: PRESETS 配列の「その他」カテゴリ以外の全プリセット（独自 CANDIDATES 配列は廃止）
  - プリセット追加で比較対象が自動的に増える
- 誤差: max(|Δre|, |Δim|) の最大値
- 一致判定: `maxError < 1e-6`（`TOLERANCE` 定数）
- **サンプル点で候補関数か EML 式がエラーになった場合、その点だけをスキップする**（以前は候補全体を除外していたが、点単位のスキップに改善）
- `MIN_VALID_POINTS = 3` 以上の有効点がない候補は除外される
...
`src/lib/i18n.ts` が翻訳機構全体を担う。

```typescript
export type Lang = 'ja' | 'en';
export const translations = { ja: {...}, en: {...} } as const satisfies Record<Lang, ...>;
export const LangContext = createContext<Lang>('ja');
export function useLang(): Translations { ... }
```

- `lang` 状態は `src/app/page.tsx` が `useState<Lang>('ja')` で保持する
- `page.tsx` のルート要素を `<LangContext.Provider value={lang}>` で囲む
- 各コンポーネントは `useLang()` を呼び出して翻訳オブジェクトを取得する
- ヘッダー右上のトグルボタン（`JA` / `EN`）でのみ `lang` を更新し、全 UI が即時切り替わる

新しい UI テキストを追加する場合は `translations.ja` と `translations.en` の両方に同じキーを追加し、コンポーネントでは `t.キー名` を使う。

---

## 式木の深さ制限

`src/lib/expr.ts` に `MAX_DEPTH = 200` が定数定義されており、`setSlot()` の先頭で以下のチェックを行う:

```typescript
if (path.length + depth(value) > MAX_DEPTH) return root;
```

- `path.length`: 挿入ポイントまでの現在の深さ
- `depth(value)`: 挿入しようとするサブツリーの深さ
- 合計が `MAX_DEPTH` を超える場合はルートをそのまま返し、更新を無視する
- ユーザーへの通知はなく、操作が無音でキャンセルされる
